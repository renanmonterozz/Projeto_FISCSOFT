...
class ComboBoxSeta(ctk.CTkComboBox):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._carregar_seta()

    def _carregar_seta(self):
        try:
            from PIL import Image, ImageTk
            imagem = Image.open(os.path.join(ASSETS_DIR, "arrow2.png"))
            imagem = imagem.resize((14, 12), Image.LANCZOS)
            self._seta_tk = ImageTk.PhotoImage(imagem)
        except Exception:
            self._seta_tk = None
        self._posicionar_seta()

    def _posicionar_seta(self):
        try:
            self._canvas.delete("seta_arrow_img")
        except Exception:
            pass
        try:
            self._canvas.itemconfig("dropdown_arrow", state="hidden")
        except Exception:
            pass
        if getattr(self, "_seta_tk", None) is None:
            return
        try:
            cx = self._current_width - (self._current_height / 2)
            cy = self._current_height / 2
            self._canvas.create_image(cx, cy, image=self._seta_tk, anchor="center",
                                     tags="seta_arrow_img")
            self._canvas.tag_bind("seta_arrow_img", "<Button-1>", self._clicked)
            self._canvas.tag_bind("seta_arrow_img", "<Enter>", self._on_enter)
            self._canvas.tag_bind("seta_arrow_img", "<Leave>", self._on_leave)
        except Exception:
            pass
...
class NotasFiscaisExterno(ctk.CTkFrame):
    ...
    def _build_action_buttons(self):
        ...
        ctk.CTkButton(
            ...
            fg_color="#0B3D0B", hover_color="#0F4A0F",
            text_color="white", border_width=0,
            font=ctk.CTkFont(size=FONTS["size_body"], weight="bold"),
            width=140,
        ...
        ctk.CTkButton(
            voltar_container, text="  Voltar", height=40, corner_radius=6,
            fg_color="#6B7280", hover_color="#4B5563",
            text_color="white", border_width=0,
            font=ctk.CTkFont(size=FONTS["size_body"], weight="bold"),
            width=140,
