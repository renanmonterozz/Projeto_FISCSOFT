...
class LocaisPage(CrudBase, ctk.CTkFrame):
    ...
    def build_table(self):
        ...
        self.build_table_header(self.table_frame, ["CEP", "Endereco", "Instituicao", "Responsavel", "Telefone"],
                                [1, 2, 2, 2, 1], has_checkbox=True)
    ...
    def _add_row(self, local):
        ...
        self.configure_data_columns(data, [1, 2, 2, 2, 1])

        ctk.CTkLabel(data, text=local["cep"],
                      font=ctk.CTkFont(size=FONTS["size_body"]),
                      text_color=COLORS["text"], anchor="w",
                      width=100).grid(row=0, column=0, sticky="w", padx=(10, 5))

        ctk.CTkLabel(data, text=local["endereco"][:40] + ("..." if len(local["endereco"]) > 40 else ""),
                      font=ctk.CTkFont(size=FONTS["size_body"]),
                      text_color=COLORS["text_muted"], anchor="w",
                      width=200).grid(row=0, column=1, sticky="w", padx=5)

        ctk.CTkLabel(data, text=local["instituicao"],
                      font=ctk.CTkFont(size=FONTS["size_body"]),
                      text_color=COLORS["text_muted"], anchor="w",
                      width=180).grid(row=0, column=2, sticky="w", padx=5)

        ctk.CTkLabel(data, text=local["responsavel"],
                      font=ctk.CTkFont(size=FONTS["size_body"]),
                      text_color=COLORS["text_muted"], anchor="w",
                      width=150).grid(row=0, column=3, sticky="w", padx=5)

        ctk.CTkLabel(data, text=local["telefone"],
                      font=ctk.CTkFont(size=FONTS["size_body"]),
                      text_color=COLORS["text_muted"], anchor="w",
                      width=120).grid(row=0, column=4, sticky="w", padx=(5, 10))
